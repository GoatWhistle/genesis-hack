...
module Config
  # Раскладка правил в хранилище: что где лежит. Само хранилище — диск или бакет —
  # спрятано за портом, поэтому одна и та же раскладка работает и локально, и в S3.
  class Catalog
    DEFAULT_CONTRACT = "space_payments"
    BASE = "base.yml"
    CONTRACTS = "contracts"
    CONTRACT = "contract.yml"
    # Имя профиля становится частью ключа в хранилище, поэтому это идентификатор,
    # а не произвольная строка: всё прочее профилем быть не может.
