...
module Repositories
  module Rules
    # Правила в бакете: та же раскладка ключей, что и на диске, только с префиксом.
    # Нужна, когда инструмент работает несколькими экземплярами: правила правятся
    # в одном месте, а видят их сразу все.
    class S3
      include Store
      ...
      # Префикс — не ключ: он приходит изнутри и может быть пустым, поэтому
      # проверку ключа к нему не применяем, только приклеиваем общий.
      # @param prefix [String]
