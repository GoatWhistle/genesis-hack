...
| `422` | в описании не нашлись обязательные роли | `{"error": "не распознаны обязательные роли: create_request, fetch_status. Правила распознавания — в app/config/base.yml, роли — в профиле контракта"}` |
...
**Правила перечитываются на каждый запрос.** Правка `app/config/base.yml` или профиля
